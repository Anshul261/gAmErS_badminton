...
import { addAttendee, addPlayer, archivePlayer, createGroup, deleteGame, endSession, joinGroup, listGroups, loadOlderSessions, loadSession, loadStats, loadWorkspace, rotateInvite, saveGame, startSession } from "@/lib/data";
import { createClient } from "@/lib/supabase/client";
import { dubaiDate } from "@/lib/scoring";
import type { Game, GameInput, Group, PlayerStats, Session, SessionData, Workspace } from "@/lib/types";
...
export function Courtside({ initialGroups, userId }: { initialGroups: Group[]; userId: string }) {
  ...
  const [invite, setInvite] = useState<{ groupId: string; code: string } | null>(null);
  ...
  const isAdmin = workspace?.members.some((member) => member.user_id === userId && member.role === "admin") ?? false;
  ...
  const poll = useEffectEvent(async (cancelled: () => boolean, workspaceDue: boolean) => {
    ...
    if (workspaceDue || !currentWorkspace) {
      try {
        currentWorkspace = await loadWorkspace(groupId);
        if (stale()) return;
        ...
        const currentGroup = currentWorkspace.group;
        setGroups((current) => current.some((group) => group.id === currentGroup.id) ? current : [...current, currentGroup]);
        setWorkspaceError("");
        ...
        // Hide records on a denied membership check, but retain drafts during network failures.
        setWorkspace(null);
        setSessions({});
        setStats(null);
        setOlderSessions([]);
    ...
    await Promise.all([
      ...
      workspaceDue && tab === "stats" ? (async () => {
        try {
          const data = await loadStats(groupId);
          if (stale()) return;
          setStats(data);
          setStatsError("");
        } catch (cause) {
  ...
  useEffect(() => {
    ...
    async function run(force = false) {
      clearTimeout(timer);
      if (cancelled || !groupId || document.hidden || !navigator.onLine) return;
  ...
  function switchGroup(id: string) {
    if (locked) return;
    if (playData && !window.confirm("Switch groups? Any unsaved game draft will be lost.")) return;
    epoch.current += 1;
    revision.current += 1;
    setGroupId(id);
    setWorkspace(null);
  ...
  async function mutate(label: string, task: () => Promise<void>, success: string) {
    ...
    try {
      ...
      if (currentEpoch === epoch.current) setError(message(cause));
    } finally {
  ...
  async function submitGroup(event: FormEvent<HTMLFormElement>) {
    ...
    if (actionLock.current || offline) return;
    const value = groupMode === "create" ? groupName.trim() : joinCode.trim();
    if (!value) { setError(groupMode === "create" ? "Give your group a name." : "Paste an invite code."); return; }
    actionLock.current = true;
    setBusy("group");
    setError("");
    ...
    try {
      ...
      } else newGroupId = await joinGroup(value);
    } catch (cause) {
    ...
    // Group creation succeeded. A failed list refresh must not invite a duplicate create.
    epoch.current += 1;
    revision.current += 1;
    setWorkspace(null);
    setSessions({});
    ...
    setGroupName("");
    setJoinCode("");
    setInvite(code ? { groupId: newGroupId, code } : null);
    setCopied(false);
    setTab("play");
    setNotice(code ? "Group created. Copy your invite code before closing it." : "You joined the group.");
    try { setGroups(await listGroups()); }
    catch (cause) { setError(`You joined successfully, but the group list could not refresh. ${message(cause)}`); }
    finally { actionLock.current = false; setBusy(""); refresh.current(); }
  }
  ...
  async function copyInvite() {
    ...
    try {
      ...
      setCopied(true);
    } catch { setError("Copy was blocked. Select the code and copy it manually."); }
  }

  const groupForm = (
    <form className="cs-group-form" onSubmit={submitGroup}>
      <div className="cs-segment cs-segment-wide" aria-label="Group setup">
        <button type="button" aria-pressed={groupMode === "create"} disabled={locked} onClick={() => { setGroupMode("create"); setError(""); }}>Create a group</button>
        <button type="button" aria-pressed={groupMode === "join"} disabled={locked} onClick={() => { setGroupMode("join"); setError(""); }}>Join friends</button>
      </div>
      {groupMode === "create" ? <div className="cs-field">
        <label htmlFor="group-name">Group name</label>
        <input id="group-name" className="cs-input" maxLength={60} placeholder="What do you call your crew?" autoComplete="off" required value={groupName} onChange={(event) => setGroupName(event.target.value)} disabled={locked} />
        <p className="cs-help">A shared place for your players, games and scores.</p>
      </div> : <div className="cs-field">
        <label htmlFor="join-code">Invite code</label>
        <input id="join-code" className="cs-input" maxLength={256} placeholder="Paste the code from your friend" autoComplete="off" autoCapitalize="none" spellCheck={false} required value={joinCode} onChange={(event) => setJoinCode(event.target.value)} disabled={locked} />
        <p className="cs-help">Codes expire after 7 days. Ask a group admin for a new one if needed.</p>
      </div>}
      {error ? <p className="cs-error" role="alert">{error}</p> : null}
      <button className="cs-button cs-button-green" type="submit" disabled={locked || offline}>{busy === "group" ? "Please wait..." : groupMode === "create" ? "Create group" : "Join group"}<ArrowRightIcon width={18} height={18} aria-hidden="true" /></button>
    </form>
  );
  ...
  return (
    <div className="cs-app">
      ...
      <header className="cs-header">
        <button type="button" className="cs-brand" onClick={() => setTab("play")} aria-label="Courtside home"><span className="cs-brand-mark" aria-hidden="true"><span /></span>courtside<span className="cs-brand-dot">.</span></button>
        <div className="cs-header-actions">
          {groups.length > 1 ? <label className="cs-group-select"><span className="cs-sr-only">Your group</span><select value={groupId} disabled={locked} onChange={(event) => switchGroup(event.target.value)}>{groups.map((group) => <option key={group.id} value={group.id}>{group.name}</option>)}</select></label> : null}
      ...
      <main id="main" className="cs-main">
        ...
        {error && !groupFormOpen && groupId ? <div className="cs-banner cs-banner-error" role="alert"><span>{error}</span><button className="cs-icon-button" aria-label="Dismiss error" onClick={() => setError("")}><Cross2Icon width={17} height={17} aria-hidden="true" /></button></div> : null}
        ...
        {!groupId ? <div className="cs-onboarding">
          <div className="cs-onboarding-intro"><p className="cs-eyebrow">LESS ADMIN. MORE BADMINTON.</p><h1>Your people.<br />Your court.<br /><em>Every game.</em></h1><p>Keep the scores. Settle the friendly rivalries. Make the next game a rematch.</p><div className="cs-decorative-court" aria-hidden="true"><span /><span /></div></div>
          <section className="cs-onboarding-form"><p className="cs-eyebrow">LET&apos;S GET YOU PLAYING</p><h2>Who&apos;s on your court?</h2><p className="cs-help">Start a group or join the friends already keeping score.</p>{groupForm}</section>
        </div> : <>
          <div className="cs-page-heading">
            <div><p className="cs-eyebrow">YOUR BADMINTON CREW</p><h1>{workspace?.group.name ?? (workspaceError ? "Group unavailable" : "Opening your court...")}</h1></div>
            <button type="button" className="cs-text-button" disabled={locked} onClick={() => { setGroupFormOpen(true); setError(""); }}><PlusIcon width={16} height={16} aria-hidden="true" />Create or join</button>
          </div>

          <nav className="cs-tabs" aria-label="Group sections">{tabs.map(({ id, label, icon: Icon }) => <button key={id} type="button" className={tab === id ? "is-active" : ""} aria-current={tab === id ? "page" : undefined} onClick={() => startTransition(() => setTab(id))}><Icon width={19} height={19} aria-hidden="true" /><span>{label}</span></button>)}</nav>

          {workspaceError ? <section className="cs-empty cs-access-error" role="alert"><h2>We couldn&apos;t open this group.</h2><p>{workspaceError}</p><p>Your membership or sign-in may have changed. Private group data is hidden until access is checked.</p><div className="cs-actions"><button className="cs-button cs-button-green" disabled={offline} onClick={() => refresh.current()}><ReloadIcon width={17} height={17} aria-hidden="true" />Try again</button><Link className="cs-button cs-button-outline" href="/login">Sign in again</Link></div></section> : !workspace ? <div className="cs-play-grid" role="status"><div><div className="cs-loading-court" aria-hidden="true"><div /><div /><div /><div /></div><p className="cs-help mt-4!">Getting your group ready...</p></div><div className="cs-score-sheet cs-loading-sheet" aria-hidden="true">{[0, 1, 2].map((row) => <div key={row}><span /><span /></div>)}</div></div> : <>
            ...
            }}><ArchiveIcon width={17} height={17} aria-hidden="true" /></button></li>)}</ul>{!roster.length ? <p className="cs-empty cs-empty-small">No names yet. Add at least two players to start a session.</p> : null}{workspace.players.some((player) => player.archived) ? <details className="cs-archived"><summary>Archived players ({workspace.players.filter((player) => player.archived).length})</summary><ul>{workspace.players.filter((player) => player.archived).map((player) => <li key={player.id}>{player.display_name}</li>)}</ul><p className="cs-help">Their names and results remain in past games.</p></details> : null}</div><aside className="cs-invite-panel"><div className="cs-invite-symbol" aria-hidden="true"><PersonIcon width={27} height={27} /><PlusIcon width={16} height={16} /></div><p className="cs-eyebrow">BRING YOUR PEOPLE</p><h3>Better with friends.</h3><p>Invite friends to this group so they can log games and keep up with the scores.</p><p className="cs-help">{workspace.members.length} {workspace.members.length === 1 ? "account has" : "accounts have"} access. {isAdmin ? "You are a group admin." : "Ask a group admin for an invite code."}</p>{isAdmin ? <><button className="cs-button cs-button-green cs-full" disabled={locked || offline} onClick={() => {
              if (!window.confirm("Generate a new invite code? The previous code will stop working. The new code expires in 7 days.")) return;
              void mutate("invite", async () => { const code = await rotateInvite(groupId); setInvite({ groupId, code }); setCopied(false); }, "New invite code generated. The previous code no longer works.");
            }}>{busy === "invite" ? "Generating..." : "Generate invite code"}<ArrowRightIcon width={17} height={17} aria-hidden="true" /></button><p className="cs-help">Shown once. Valid for 7 days. A new code replaces the old one.</p></> : null}</aside></div></section>
          </>}
        </>}
        <footer className="cs-footer"><span>courtside.</span><span>For the love of the game.</span></footer>
      </main>

      <Dialog open={groupFormOpen} onOpenChange={(open) => { if (!locked) { setGroupFormOpen(open); setError(""); } }}><DialogContent className="cs-dialog" showCloseButton={!locked}><DialogHeader><DialogTitle>Find your court</DialogTitle><DialogDescription>Create a group or join one with an invite code.</DialogDescription></DialogHeader>{groupForm}</DialogContent></Dialog>
      <Dialog open={Boolean(invite && invite.groupId === groupId)} onOpenChange={(open) => { if (!open) setInvite(null); }}><DialogContent className="cs-dialog"><DialogHeader><DialogTitle>Your group&apos;s invite code</DialogTitle><DialogDescription>Share this code privately with your friends. It expires in 7 days and is only shown here once.</DialogDescription></DialogHeader><div className="cs-invite-code"><code>{invite?.code}</code><button className="cs-button cs-button-green" onClick={() => void copyInvite()}>{copied ? <CheckIcon width={17} height={17} aria-hidden="true" /> : <CopyIcon width={17} height={17} aria-hidden="true" />}{copied ? "Copied" : "Copy code"}</button></div><p className="cs-help">Friends can paste it into Join friends after signing in. Anyone with the code can join.</p><p role="status" className="cs-sr-only">{copied ? "Invite code copied to clipboard." : ""}</p>{error ? <p className="cs-error" role="alert">{error}</p> : null}<button className="cs-button cs-button-outline" onClick={() => setInvite(null)}>I&apos;ve saved the code</button></DialogContent></Dialog>
