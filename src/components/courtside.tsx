...
const faqs: [string, string][] = [
  ...
  ["How the leaderboard works", "Players are ranked by Score. Every game is worth up to 2 points: between evenly matched sides a win is +1 and a loss is -1. Beat a stronger side and the win is worth more; lose to a weaker side and it costs more. A side's strength is the average score of its players, so winning with a strong partner counts a little less and winning with a weaker partner counts more. Margin of victory is ignored: 11-9 and 11-0 are both just a win. Win % and Pts +/- break ties."],
  ["Does a 1v2 hurt my score?", "No. If you're in the pair it's scored like any other game: a win is a full win. If you're the solo player you get a handicap, so a win is worth about 1.3 and a loss costs about 0.7. Nobody comes out behind for agreeing to a 1v2."],
  ["Why did that game count +1.4 and this one only +0.6?", "Each game on the score sheet shows its value. It's higher when your side was the underdog going in and lower when you were the favourites, based on everyone's score at the time."],
...
export function Courtside({ userId, defaultName }: { userId: string; defaultName: string }) {
  ...
  const gameValue = (data: SessionData, game: Game, playerId: string) => data.values[game.id]?.[playerId];
    ...
    return <div className="cs-game-list">
      {games.length ? <ol>
        {games.slice(0, gameLimit).map((game, index) => {
          ...
          const valueA = gameValue(data, game, game.side_a_player_1);
          const valueB = gameValue(data, game, game.side_b_player_1);
          return <li className="cs-game-row" key={game.id}>
            ...
            <div className={`cs-result-side ${game.score_a > game.score_b ? "is-winner" : ""}`}><span>{sideA}{game.score_a > game.score_b ? <span className="cs-sr-only">, winner</span> : null}{valueA !== undefined ? <small className={`cs-game-value ${valueA > 0 ? "is-gain" : ""}`} title="Leaderboard score for this game">{signed(valueA)}</small> : null}</span><strong>{game.score_a}</strong></div>
            <div className={`cs-result-side ${game.score_b > game.score_a ? "is-winner" : ""}`}><span>{sideB}{game.score_b > game.score_a ? <span className="cs-sr-only">, winner</span> : null}{valueB !== undefined ? <small className={`cs-game-value ${valueB > 0 ? "is-gain" : ""}`} title="Leaderboard score for this game">{signed(valueB)}</small> : null}</span><strong>{game.score_b}</strong></div>
    ...
    <div className="cs-app">
      ...
      <main id="main" className="cs-main">
        ...
        {workspaceError ? <section className="cs-empty cs-access-error" role="alert"><h2>We couldn&apos;t open the court.</h2><p>{workspaceError}</p><p>Your sign-in may have expired. Scores are hidden until access is checked.</p><div className="cs-actions"><button className="cs-button cs-button-green" disabled={offline} onClick={() => refresh.current()}><ReloadIcon width={17} height={17} aria-hidden="true" />Try again</button><Link className="cs-button cs-button-outline" href="/login">Sign in again</Link></div></section> : !workspace ? <div className="cs-play-grid" role="status"><div><div className="cs-loading-court" aria-hidden="true"><div /><div /><div /><div /></div><p className="cs-help mt-4!">Getting the court ready...</p></div><div className="cs-score-sheet cs-loading-sheet" aria-hidden="true">{[0, 1, 2].map((row) => <div key={row}><span /><span /></div>)}</div></div> : <>
          ...
          <section hidden={tab !== "stats"} aria-label="Stats"><div className="cs-session-heading"><div><p className="cs-eyebrow">A LITTLE FRIENDLY COMPETITION</p><h2>The numbers don&apos;t lie.</h2></div><span className="cs-caption">All sessions</span></div><p className="cs-stats-note">Ranked by <strong>Score</strong>: every game is worth up to 2 points depending on how strong each side was. Beat a stronger side and it counts for more; a strong partner makes a win count a little less. In a 1v2 the pair is scored like any game and the solo player gets a handicap. Margin of victory is ignored.</p>{statsError ? <div className="cs-banner cs-banner-warning" role="alert"><span>Stats may not be up to date. {statsError}</span><button className="cs-text-button" disabled={offline} onClick={() => refresh.current()}>Retry</button></div> : null}{stats === null ? <div className="grid gap-4 py-4" role="status">{!statsError ? [0, 1, 2].map((row) => <div key={row} aria-hidden="true" className="grid h-12 grid-cols-[2fr_repeat(4,1fr)] gap-3 border-b pb-3"><span className="rounded bg-muted" /><span className="rounded bg-muted" /><span className="rounded bg-muted" /><span className="rounded bg-muted" /><span className="rounded bg-muted" /></div>) : null}<p className="cs-help">{statsError ? "Stats are unavailable." : "Counting the games..."}</p></div> : !stats.some((player) => player.played > 0) ? <div className="cs-empty"><BarChartIcon width={38} height={38} aria-hidden="true" /><h3>Bragging rights start with game one.</h3><p>Log a game to see wins, win rates and point differences for your players.</p><button className="cs-button cs-button-green" onClick={() => setTab("play")}>Back to the court<ArrowRightIcon width={17} height={17} aria-hidden="true" /></button></div> : <div className="cs-stats-table-wrap"><table className="cs-stats-table"><caption className="cs-sr-only">Player statistics across all sessions, ranked by strength-adjusted score. 1v2 shows games played solo and games played as the pair. Points difference is points scored minus points conceded.</caption><thead><tr><th scope="col">Player</th><th scope="col">Score</th><th scope="col"><span aria-label="Wins and losses">W–L</span></th><th scope="col">Win %</th><th scope="col" className="cs-formats-head"><span aria-label="Games played as 1v1, 1v2 solo or pair, and 2v2">1v1·1v2·2v2</span></th><th scope="col"><span aria-label="Points difference">Pts +/-</span></th></tr></thead><tbody>{[...stats].sort((a, b) => b.score - a.score || (b.wins / Math.max(b.played, 1)) - (a.wins / Math.max(a.played, 1)) || (b.points_for - b.points_against) - (a.points_for - a.points_against)).map((player, index) => {
            ...
            return <tr key={player.player_id}><th scope="row"><span className="cs-rank">{String(index + 1).padStart(2, "0")}</span><span>{names.get(player.player_id) ?? "Archived player"}{archived ? <small>Archived</small> : null}</span></th><td className={`cs-stat-score ${player.score > 0 ? "cs-positive" : ""}`}>{signed(player.score)}</td><td><span className="cs-stat-wins">{player.wins}</span><span className="cs-dim">–</span>{player.losses}</td><td>{player.played ? `${Math.round(player.wins / player.played * 100)}%` : "0%"}</td><td className="cs-formats"><span title="1v1">{player.singles}</span><span className="cs-dim">·</span><span title="1v2: solo / pair">{player.solo}<span className="cs-dim">/</span>{player.pair}</span><span className="cs-dim">·</span><span title="2v2">{player.doubles}</span></td><td className={difference > 0 ? "cs-positive" : ""}>{difference > 0 ? "+" : ""}{difference}</td></tr>;
          })}</tbody></table></div>}<p className="cs-help cs-stats-footnote">Formats are games played as 1v1 · 1v2 (solo / pair) · 2v2. Pts +/- is points scored minus points conceded and breaks ties after win %. Archived players keep their results. Each game&apos;s score value is shown next to it on the score sheet.</p></section>
